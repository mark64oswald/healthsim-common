...
def temp_db():
    ...
    """)
    conn.execute("CREATE SEQUENCE scenario_entities_seq START 1")
...
class TestAddEntitiesToExisting:
    ...
    def test_add_different_entity_types(self, connection_manager):
        ...
        with connection_manager.write_connection() as conn:
            ...
            types = conn.execute("""
                SELECT DISTINCT entity_type FROM scenario_entities WHERE scenario_id = ?
...
class TestUpsertBehavior:
    ...
    def test_insert_new_entity(self, connection_manager):
        ...
        with connection_manager.write_connection() as conn:
            ...
            # Verify insert
            result = conn.execute("""
                SELECT entity_data FROM scenario_entities 
