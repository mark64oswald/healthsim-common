...
DIABETIC_SENIOR = PatientProfileSpecification(
    ...
    demographics=PatientDemographicsSpec(
        age={"type": "normal", "mean": 72, "std": 8, "min": 65, "max": 95},
        gender={"type": "categorical", "weights": {"M": 0.48, "F": 0.52}},
    ),
    clinical=PatientClinicalSpec(
        primary_condition={"code": "E11.9", "name": "Type 2 diabetes mellitus"},
        comorbidities=[
            {"code": "I10", "name": "Essential hypertension", "prevalence": 0.75},
            {"code": "E78.5", "name": "Hyperlipidemia", "prevalence": 0.65},
            {"code": "I25.10", "name": "Atherosclerotic heart disease", "prevalence": 0.30},
            {"code": "N18.3", "name": "Chronic kidney disease, stage 3", "prevalence": 0.25},
            {"code": "E66.9", "name": "Obesity", "prevalence": 0.40},
        ],
        encounter_pattern=EncounterSpec(
            encounter_class="outpatient",
            frequency={"type": "normal", "mean": 6, "std": 2, "min": 2},
        ),
    ),
...
HEALTHY_ADULT = PatientProfileSpecification(
    ...
    clinical=PatientClinicalSpec(
        encounter_pattern=EncounterSpec(
            encounter_class="outpatient",
            frequency={"type": "normal", "mean": 2, "std": 1, "min": 0},
        ),
    ),
    generation=PatientGenerationSpec(
        count=100,
...
PEDIATRIC_ASTHMA = PatientProfileSpecification(
    ...
    clinical=PatientClinicalSpec(
        primary_condition={"code": "J45.20", "name": "Mild intermittent asthma"},
        comorbidities=[
            {"code": "J30.9", "name": "Allergic rhinitis", "prevalence": 0.60},
            {"code": "L20.9", "name": "Atopic dermatitis", "prevalence": 0.30},
        ],
        encounter_pattern=EncounterSpec(
            encounter_class="outpatient",
            frequency={"type": "normal", "mean": 4, "std": 2, "min": 1},
        ),
    ),
    generation=PatientGenerationSpec(
        count=100,
...
ED_FREQUENT_FLYER = PatientProfileSpecification(
    ...
    demographics=PatientDemographicsSpec(
        age={"type": "normal", "mean": 45, "std": 15, "min": 18, "max": 75},
        gender={"type": "categorical", "weights": {"M": 0.55, "F": 0.45}},
    ),
    clinical=PatientClinicalSpec(
        primary_condition={"code": "F10.20", "name": "Alcohol dependence, uncomplicated"},
        comorbidities=[
            {"code": "F32.9", "name": "Major depressive disorder", "prevalence": 0.50},
            {"code": "F41.1", "name": "Generalized anxiety disorder", "prevalence": 0.40},
            {"code": "G43.909", "name": "Migraine, unspecified", "prevalence": 0.35},
            {"code": "M54.5", "name": "Low back pain", "prevalence": 0.55},
        ],
        encounter_pattern=EncounterSpec(
            encounter_class="emergency",
            frequency={"type": "normal", "mean": 8, "std": 4, "min": 4},
        ),
    ),
    generation=PatientGenerationSpec(
        count=50,
...
SURGICAL_INPATIENT = PatientProfileSpecification(
    ...
    demographics=PatientDemographicsSpec(
        age={"type": "normal", "mean": 58, "std": 12, "min": 30, "max": 80},
        gender={"type": "categorical", "weights": {"M": 0.52, "F": 0.48}},
    ),
    clinical=PatientClinicalSpec(
        comorbidities=[
            {"code": "I10", "name": "Essential hypertension", "prevalence": 0.45},
            {"code": "E11.9", "name": "Type 2 diabetes mellitus", "prevalence": 0.25},
            {"code": "E66.9", "name": "Obesity", "prevalence": 0.35},
        ],
        encounter_pattern=EncounterSpec(
...
MATERNITY = PatientProfileSpecification(
    ...
    demographics=PatientDemographicsSpec(
        age={"type": "normal", "mean": 30, "std": 5, "min": 18, "max": 45},
        gender={"type": "categorical", "weights": {"F": 1.0}},
        mrn_prefix="OB",
    ),
    clinical=PatientClinicalSpec(
        primary_condition={"code": "Z34.00", "name": "Supervision of normal pregnancy"},
        comorbidities=[
            {"code": "O24.410", "name": "Gestational diabetes mellitus", "prevalence": 0.15},
            {"code": "O13.9", "name": "Gestational hypertension", "prevalence": 0.10},
            {"code": "O99.810", "name": "Abnormal glucose complicating pregnancy", "prevalence": 0.08},
        ],
        encounter_pattern=EncounterSpec(
            encounter_class="outpatient",
            frequency={"type": "normal", "mean": 13, "std": 2, "min": 10},  # Prenatal visits
        ),
    ),
    generation=PatientGenerationSpec(
        count=100,
...
ONCOLOGY = PatientProfileSpecification(
    ...
    demographics=PatientDemographicsSpec(
        count=100,
        age={"type": "normal", "mean": 65, "std": 12, "min": 30, "max": 90},
        gender={"type": "categorical", "weights": {"M": 0.48, "F": 0.52}},
        mrn_prefix="ONC",
    ),
    clinical=PatientClinicalSpec(
        primary_condition={"code": "C34.90", "name": "Malignant neoplasm of lung"},
        comorbidities=[
            {"code": "J44.9", "name": "COPD", "prevalence": 0.35},
            {"code": "D64.9", "name": "Anemia", "prevalence": 0.45},
            {"code": "R53.83", "name": "Fatigue", "prevalence": 0.70},
            {"code": "R63.0", "name": "Anorexia", "prevalence": 0.40},
        ],
        encounter_pattern=EncounterSpec(
            encounter_class="outpatient",
            frequency={"type": "normal", "mean": 18, "std": 6, "min": 6},
        ),
    ),
    generation=PatientGenerationSpec(
        include_encounters=True,
